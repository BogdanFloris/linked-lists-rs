...
impl<T> LinkedList<T> {
    ...
    pub fn push_front(&mut self, elem: T) {
        unsafe {
            let to_add = NonNull::new_unchecked(Box::into_raw(Box::new(Node {
                front: None,
                back: None,
                elem,
            })));
            if let Some(old) = self.front {
                (*old.as_ptr()).front = Some(to_add);
                (*to_add.as_ptr()).back = Some(old);
            } else {
                self.back = Some(to_add);
            }
            self.front = Some(to_add);
            self.len += 1;
        }
    }
    ...
    pub fn push_back(&mut self, elem: T) {
        unsafe {
            ...
            if let Some(old) = self.back {
                (*old.as_ptr()).back = Some(to_add);
                (*to_add.as_ptr()).front = Some(old);
            } else {
                self.front = Some(to_add);
            }
            self.back = Some(to_add);
